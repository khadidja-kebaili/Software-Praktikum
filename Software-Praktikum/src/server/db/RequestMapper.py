...
class RequestMapper(Mapper):
    ...
    def find_all(self):
        ...
        for (id, requested_by, requested, request_date,
             request_type, group_id) in tuples:
            request = Request()
            request.set_id(id)
    ...
    def find_by_key(self, key):
        ...
        cursor = self._cnx.cursor()
        command = "SELECT id, requested_by, requested, request_date, request_type, group_id FROM request " \
                  "WHERE id={}".format(key)
