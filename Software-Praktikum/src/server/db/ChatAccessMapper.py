...
class ChatAccessMapper(Mapper):
    ...
    def find_by_key(self, id):
        res = None
        cursor = self._cnx.cursor()
        command = "SELECT id, profilID, room, chattype FROM chataccess WHERE id={}".format(
            id)
    ...
    def find_groupchat_by_profil(self, profilID):
        res = []
        cursor = self._cnx.cursor()
        command = "SELECT id, profilID, room, chattype FROM chataccess WHERE profilID={} AND chattype='g'".format(
            profilID)
        cursor.execute(command)
        tuples = cursor.fetchall()
    ...
    def find_singlechat_by_profil(self, profilID):
        res = []
        cursor = self._cnx.cursor()
        command = "SELECT id, profilID, room, chattype FROM chataccess WHERE profilID={} AND chattype='e'".format(
            profilID)
        cursor.execute(command)
        tuples = cursor.fetchall()
