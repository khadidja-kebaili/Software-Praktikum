...
class UserMapper (Mapper):
    ...
    def find_by_name(self, name):
        ...
        result = []
        cursor = self._cnx.cursor()
        command = "SELECT id, name, email FROM users WHERE name LIKE '{}' ORDER BY name".format(name)
    ...
    def find_by_email(self, mail_address):
        ...
        cursor = self._cnx.cursor()
        command = "SELECT id, name, email FROM users WHERE email={}".format(mail_address)
        cursor.execute(command)
        tuples = cursor.fetchall()
    ...
    def update(self, user):
        ...
        command = "UPDATE users " + "SET name=%s, email=%s WHERE user_id=%s"
